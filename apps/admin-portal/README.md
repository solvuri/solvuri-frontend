...
An internal operator console: a login screen and a dashboard covering Global Stats, Merchants, Catalog, Clearack Stores, POS Locations, and Platform Settings. Auth, Global Stats (real platform revenue), Merchants (onboarding + M-Pesa verify/enable + subscription billing), Catalog (System Category/Feature CRUD), Clearack Stores, and POS Locations (both real merchant/subscription data) are wired to the real backend; only Platform Settings still runs on local component state. See `AGENTS.md` for the exact split.
