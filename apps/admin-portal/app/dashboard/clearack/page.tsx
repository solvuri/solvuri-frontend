...
interface ClearackStore {
  tenant: TenantSummary;
  subscription: MerchantSubscriptionSummary;
}

export default function ClearackStoresPage() {
  const [stores, setStores] = useState<ClearackStore[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const clearackStores: ClearackStore[] = [];
  ...
  return (
    <div className="p-8">
      <h1 className="text-4xl font-bebas text-text mb-8">Clearack Stores</h1>
      ...
      <div className="bg-surface rounded-2xl border border-primary/10 p-8 mb-8">
        {isLoading ? (
          ...
          <p className="text-muted text-sm">No Clearack stores yet.</p>
        ) : (
