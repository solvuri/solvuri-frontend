// // apps/clearack/lib/clearackApi.ts
...
export function useProductsByDomain(domain: string | null) {
  return useQuery({
    queryKey: ["clearack-products-domain", domain],
    queryFn: () => fetchProductsByDomain(domain as string),
    enabled: domain !== null && domain !== "",
  });
}

// GET /api/clearack/products/inventory — authenticated / merchant dashboard
export function fetchInventory(): Promise<ClearackProduct[]> {
  return clearackApi
    .get<ClearackProduct[]>("/api/clearack/products/inventory")
    .then((res) => res.data);
}

export function useInventory() {
  return useQuery({
    queryKey: ["clearack-inventory"],
